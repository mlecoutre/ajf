...
/**
 * The services repository manage the services interface and implementations
 * found in the application. It will contain for each interface the couple :
 * (interface, implementations *) The implementations will only be the last
 * children if inheritence is used in implementations.
 * 
 * The methods to use the repository are only available when the scan is
 * complete.
 * 
...
public class ServicesRepository {
	...
	private static final Logger logger = LoggerFactory
			.getLogger(ServicesRepository.class);
	// private static ServicesRepository instance;

	/*
	 * public static ServicesRepository getInstance() { if (instance == null) {
	 * instance = new ServicesRepository(); } return instance; }
	 */
	...
	void completeScan() {
		...
		logger.info("Services scan finished : found " + nbInterfaces
				+ " interface(s) and " + nbImpls + " implementation(s).");
	}
	...
	public void addService(Class<?> service) throws MalformedServiceException {
		...
		if (isServiceInterface(service)) {
			...
			if (isService(service)) {
				// get the BD interface for the service, there should be only
				// one
				...
				if (impls == null) {
					...
					// The second simple case, the interface was already
					// scanned, but no other impls
				}
				else
					if (impls.size() == 0) {
						impls.add(service);
						...
						// TODO need to manage the cases when there is
						// inheritence between services on the client side
						boolean vetoAdd = false;
						for (Class<?> clazz : impls) {
							// remove the existing if we are about to add a
							// subclass of an existing impl
							if (clazz.isAssignableFrom(service)) {
								impls.remove(clazz);
								break;
							}
							// if we are about to add a parent, then veto the
							// add
	...
	public List<Class<?>> getServicesForInterface(Class<?> pInterface)
	...
	public boolean isImplemented(Class<?> pInterface)
			throws NotInitializedException {
		if (isRepositoryScanComplete()) {
