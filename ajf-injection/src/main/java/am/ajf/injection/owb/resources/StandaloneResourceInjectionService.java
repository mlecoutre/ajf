 ...
 * under the License.
 */
package am.ajf.injection.owb.resources;
...
public class StandaloneResourceInjectionService implements
	...
	/**
	 * When ResourceProxyHandler deserialized, this will instruct owb to create
	 * a new actual instance, if the actual resource is not serializable.
	 */
	private static final String DUMMY_STRING = "owb.actual.resource.dummy";

	private final StandaloneResourceProcessor processor = StandaloneResourceProcessor
			.getProcessor();

	private static WebBeansLogger logger = WebBeansLogger
			.getLogger(StandaloneResourceInjectionService.class);
	...
	public <X, T extends Annotation> X getResourceReference(
		...
		if (resourceReference.supports(Resource.class)) {
			...
			return processor.getResource(resource,
					resourceReference.getResourceType());
		}

		if (resourceReference.supports(WebServiceRef.class)) {
			WebServiceRef resource = resourceReference
					.getAnnotation(WebServiceRef.class);
			return processor.getWebServiceResource(resource,
					resourceReference.getResourceType());

		}

		if (resourceReference.supports(PersistenceContext.class)) {
			PersistenceContext persistenceContext = resourceReference
					.getAnnotation(PersistenceContext.class);
			return processor.getEntityManager(persistenceContext,
					resourceReference.getResourceType());
		}

		if (resourceReference.supports(PersistenceUnit.class)) {
			PersistenceUnit persistenceUnit = resourceReference
					.getAnnotation(PersistenceUnit.class);
			return processor.getEntityManagerFactory(persistenceUnit,
					resourceReference.getResourceType());
		}
	...
	public void injectJavaEEResources(Object managedBeanInstance) {
		...
		if (containsEeResource != null
				&& containsEeResource.booleanValue() == false) {
		...
		while (currentClass != null
			...
			Field[] fields = webBeansContext.getSecurityService()
					.doPrivilegedGetDeclaredFields(currentClass);

			for (Field field : fields) {
				if (!field.isAnnotationPresent(Produces.class)) {
					if (!Modifier.isStatic(field.getModifiers())) {
						Annotation ann = AnnotationUtil
								.hasOwbInjectableResource(field
										.getDeclaredAnnotations());
						if (ann != null) {
							try {
								if (null == field.get(managedBeanInstance)) {
									@SuppressWarnings({ "unchecked", "rawtypes" })
									ResourceReference<Object, ?> resourceRef = new ResourceReference(
								...
								throw new WebBeansException(
										MessageFormat.format(
												logger.getTokenString(OWBLogConst.ERROR_0025),
												field), e);
							}
						}
					}
	...
	public <T> void writeExternal(Bean<T> bean, T actualResource,
		...
		FailOverService failoverService = webBeansContext
				.getService(FailOverService.class);
	...
	public <T> T readExternal(Bean<T> bean, ObjectInput in) throws IOException,
		...
		FailOverService failoverService = webBeansContext
				.getService(FailOverService.class);
		if (failoverService != null) {
			actualResource = (T) failoverService.handleResource(bean,
