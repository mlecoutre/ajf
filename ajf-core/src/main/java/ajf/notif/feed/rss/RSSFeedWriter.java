...
public class RSSFeedWriter {
	...
	public RSSFeedWriter(OutputStream outputStream) {
		super();
		this.outputStream = outputStream;
	}

	public void generateToFile(RSS rss) throws IOException {
	...
	private static Document generateDocument(RSS rssContent) {
		...
		try {
			...
			Element chan;
			for (Iterator<Channel> channels = rssContent.getChannels()
					.iterator(); channels.hasNext(); root.appendChild(chan)) {
				...
				if (channel.getPubDate() != null) {
					org.w3c.dom.Text content = doc.createTextNode(sdf
				...
				{
					Element elem;
					for (Iterator<Category> categories = channel
							.getCategories().iterator(); categories.hasNext(); chan
							.appendChild(elem)) {
				...
				if (channel.getCloud() != null) {
					...
					elem.setAttribute("registerProcedure",
				...
				if (channel.getImage() != null) {
					...
					org.w3c.dom.Text titlect = doc.createTextNode(image
							.getTitle());
					Element title = doc.createElement("title");
					url.appendChild(titlect);
					elem.appendChild(title);
					org.w3c.dom.Text linkct = doc.createTextNode(image
							.getLink());
					Element link = doc.createElement("link");
					url.appendChild(linkct);
					elem.appendChild(link);
					if (image.getWidth() > 0) {
						org.w3c.dom.Text wct = doc.createTextNode(String
								.valueOf(image.getWidth()));
						Element w = doc.createElement("width");
						w.appendChild(wct);
						elem.appendChild(w);
					}
					if (image.getHeight() > 0) {
						org.w3c.dom.Text hct = doc.createTextNode(String
								.valueOf(image.getHeight()));
				...
				if (channel.getTextInput() != null) {
					...
					org.w3c.dom.Text content = doc.createTextNode(tinput
							.getTitle());
				...
				if (channel.getSkipHours() != null) {
					...
					StringTokenizer hours = new StringTokenizer(
							channel.getSkipHours(), ",");
					label0: while (hours.hasMoreTokens()) {
						String hr = hours.nextToken().trim();
						if (hr.indexOf("-") != -1) {
							int x = Integer.parseInt(hr.substring(0,
									hr.indexOf("-")));
							int y = Integer.parseInt(hr.substring(hr
									.indexOf("-") + 1));
							do {
								Element hour = doc.createElement("hour");
								org.w3c.dom.Text hourct = doc
										.createTextNode(String.valueOf(x % 24));
				...
				if (channel.getSkipDays() != 0 && channel.getSkipDays() != -128) {
					...
					if ((days & 8) == 8) {
						Element day = doc.createElement("day");
						org.w3c.dom.Text dayct = doc
								.createTextNode("Wednesday");
						day.appendChild(dayct);
						skip.appendChild(day);
					}
					if ((days & 0x10) == 16) {
				...
				Element itelem;
				for (Iterator<Item> items = channel.getItems().iterator(); items
						.hasNext();) {
					Item item = (Item) items.next();
					if (null != item) {
						itelem = doc.createElement("item");
