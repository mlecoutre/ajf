package am.ajf.core.services.utils;
...
public class ServiceUtils {
	...
	public static void handlerError(Logger log, String message,
		...
		if (cause != null) {
			if (cause instanceof AbstractLayerException) {
				((AbstractLayerException) cause).setAlreadyHandled(true);
				/*
				AbstractLayerException layerException = (AbstractLayerException ) cause;
				propagatedCause = cause;
				propagatedCause.setStackTrace(null);
				propagatedCause.setc
					new PersistenceLayerException(pLayerException.getMessage(),
