...
public class TransactionInterceptor {
	...
	/**
	 * Add the transaction logic to the method.
	 * Dont start a transaction if @NoTransaction is present.
	 * The commit workflow is as follow :
	 * <ol>
	...
	public Object manageTransaction(InvocationContext ctx) throws Throwable {
		boolean noTranOnMethod = ctx.getMethod().isAnnotationPresent(NonTransactional.class);
		boolean noTranOnClass = ctx.getMethod().getDeclaringClass().isAnnotationPresent(NonTransactional.class);
		Object res;
		...
		if (noTranOnClass || noTranOnMethod) {
			...
			try {
				...
				if (utx.getStatus() == Status.STATUS_MARKED_ROLLBACK) {
					logger.debug("Rolling back Transaction (set as rollback only) : "+utx.getStatus());
					utx.rollback();
