...
import am.ajf.notif.feed.rss.model.Category;
import am.ajf.notif.feed.rss.model.Channel;
import am.ajf.notif.feed.rss.model.Cloud;
import am.ajf.notif.feed.rss.model.Enclosure;
import am.ajf.notif.feed.rss.model.Guid;
import am.ajf.notif.feed.rss.model.Image;
import am.ajf.notif.feed.rss.model.Item;
import am.ajf.notif.feed.rss.model.RSS;
...
public class RSSFeedWriter {
	...
	public void write(OutputStream outputStream) throws IOException {
		...
		try {
			...
			String message = "Unable to write RSS document, exception is: "
					.concat(e.getMessage());
	...
	private void writeItems(Document doc, Element chan, List<Item> itemsList) {
		for (Iterator<Item> items = itemsList.iterator(); items.hasNext();) {
	...
	private void writeItem(Document doc, Element chan, Item item) {
		...
		if (item.getTitle() != null) {
			org.w3c.dom.Text content = doc.createTextNode(item.getTitle());
			Element elem = doc.createElement("title");
			elem.appendChild(content);
			itelem.appendChild(elem);
		}
		if (item.getLink() != null) {
			org.w3c.dom.Text content = doc.createTextNode(item.getLink());
		...
		if (item.getDescription() != null) {
			org.w3c.dom.Text content = doc
					.createTextNode(item.getDescription());
			Element elem = doc.createElement("description");
			elem.appendChild(content);
			itelem.appendChild(elem);
		}
		if (item.getAuthor() != null) {
			org.w3c.dom.Text content = doc.createTextNode(item.getAuthor());
		...
		{
			Element elem;
			for (Iterator<Category> icategories = item.getCategories()
					.iterator(); icategories.hasNext(); itelem
					.appendChild(elem)) {
				Category category = (Category) icategories.next();
		...
		if (item.getComments() != null) {
			org.w3c.dom.Text content = doc.createTextNode(item.getComments());
		...
		if (item.getGuid() != null) {
			...
			if (!guid.isPermaLink()) elem.setAttribute("isPermaLink", "false");
			org.w3c.dom.Text content = doc.createTextNode(guid.getId());
			elem.appendChild(content);
			itelem.appendChild(elem);
		}
