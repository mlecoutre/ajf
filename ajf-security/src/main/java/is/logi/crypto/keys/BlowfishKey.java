 ...
 * <p>
 * The CDS for a Blowfish key is <code>BlowfishKey(key)</code> with
 * <code>key</code> a string of hexadecimal digits to create a specific
 * key or <code>BlowfishKey(?)</code> for a random BlowfishKey object.
 *
 * @author <a href="http://www.hi.is/~logir/">Logi Ragnarsson</a>
 * (<a href="mailto:logir@hi.is">logir@hi.is</a>)
 */
public class BlowfishKey extends SymmetricKey implements CipherKey {
    ...
    /** Create a new Blowfish key with the key bits from <code>key</code>. */
    ...
    private void buildSubKeys(){
        ...
        byte[] block=new byte[8];  // All-zero array
        fillArray(P , block);
        fillArray(S1, block);
        fillArray(S2, block);
        fillArray(S3, block);
    ...
    public void decrypt(byte[] source, int i, byte[] dest, int j){
        int L1 = makeInt(source,i  ,4);
        int R1 = makeInt(source,i+4,4);
        int L2,R2;

        for(int r=17; r>1; r-=2){
            R2 = L1^P[r  ];
            L2 = R1^(((S1[(R2>>24)&0xff] + S2[(R2>>16)&0xff]) ^ S3[(R2>>8)&0xff]) + S4[R2&0xff]);
            R1 = L2^P[r-1];
            L1 = R2^(((S1[(R1>>24)&0xff] + S2[(R1>>16)&0xff]) ^ S3[(R1>>8)&0xff]) + S4[R1&0xff]);
        }

        writeBytes(R1^P[0], dest,j+0, 4);
        writeBytes(L1^P[1], dest,j+4, 4);
    }
    /**
    ...
    public void encrypt(byte[] source, int i, byte[] dest, int j){
        ...
        for(int r=0; r<16; r+=2){
            R2 = L1^P[r  ];
            L2 = R1^(((S1[(R2>>24)&0xff] + S2[(R2>>16)&0xff]) ^ S3[(R2>>8)&0xff]) + S4[R2&0xff]);
  	    //System.out.println(P[r  ]+" "+L2+" "+R2);
            R1 = L2^P[r+1];
            L1 = R2^(((S1[(R1>>24)&0xff] + S2[(R1>>16)&0xff]) ^ S3[(R1>>8)&0xff]) + S4[R1&0xff]);
  	    //System.out.println(P[r+1]+" "+L1+" "+R1);
        }

        writeBytes(R1^P[17], dest,j+0, 4);
        writeBytes(L1^P[16], dest,j+4, 4);
    }
    ...
    public boolean equals(Object o){
        ...
        if (o.getClass() != this.getClass())
            return false;
        return equal(key,((BlowfishKey)o).key);
    }
    /** Repeatedly encrypt block and but the output into array. */
    private void fillArray(int[] array, byte[] block){
        int n = array.length;
        for(int j=0; j<n;){
	    //System.out.println("-------- "+j+" --------");
            encrypt(block,0, block,0);
            //System.out.println(makeInt(block,0,4)+" "+makeInt(block,4,4));
	    int x;
	    x =                   (block[0] & 0xff);
	    x =          (x<<8) | (block[1] & 0xff);
	    x =          (x<<8) | (block[2] & 0xff);
	    array[j++] = (x<<8) | (block[3] & 0xff);
	    x =                   (block[4] & 0xff);
	    x =          (x<<8) | (block[5] & 0xff);
	    x =          (x<<8) | (block[6] & 0xff);
	    array[j++] = (x<<8) | (block[7] & 0xff);
        }
    }
    ...
    public static BlowfishKey parseCDS(String key) throws InvalidCDSException{
        ...
        while((i<key.length()) && Character.isWhitespace(key.charAt(i)))
            i++;
        if(i==key.length())
            throw new InvalidCDSException("Empty argument in BlowfishKey(String)");
        int j=key.length()-1;
        ...
        if(j==i && key.charAt(i)=='?')
            ...
            return new BlowfishKey(fromHexString(key.substring(i,j+1)));
    }
    /** The block-size for the Blowfish cipher is 8 bytes. */
    ...
    public String toString(){
        return "BlowfishKey("+hexString(key)+")";
    }
}
