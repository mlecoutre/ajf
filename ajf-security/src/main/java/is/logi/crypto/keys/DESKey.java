 ...
 * complete security.
 * <p>
 * The CDS for a DES key is <code>DESKey(key)</code> with <code>key</code>
 * a string of 16 hexadecimal digits to create a specific key
 * or <code>DESKey(?)</code> for a random DESKey object.
 *
 * @see is.logi.crypto.keys.TriDESKey
 *
 * @author <a href="http://www.hi.is/~logir/">Logi Ragnarsson</a>
 * (<a href="mailto:logir@hi.is">logir@hi.is</a>)
 */
public class DESKey extends SymmetricKey implements CipherKey {
    ...
    public static DESKey parseCDS(String key) throws InvalidCDSException{
        ...
        if(j==i && key.charAt(i)=='?')
            ...
            //this.key = new java.math.BigInteger(key.substring(i,j+1),16).longValue();
            k = makeLong(fromHexString(key.substring(i,j+1)),0,8);
